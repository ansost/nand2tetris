...
def compile_varName(token_list, current_idx, xml, subroutine_symbols, use_compile_type = None):
    if use_compile_type:
    ...
    if tracker.get_token() in [";"]:
        ...
        return token_list, current_idx, xml, subroutine_symbols
    elif tracker.get_token() in [")"]:
        return token_list, current_idx, xml, subroutine_symbols
    elif tracker.get_token() == ",":
        tracker.advance() # write comma
        return compile_varName(token_list, current_idx, xml, subroutine_symbols=subroutine_symbols, use_compile_type = use_compile_type)
    
def compile_varName_classVarDec(token_list, current_idx, xml, global_symbols):
    global_symbols["name"].append(tracker.get_token())
    tracker.advance() # write varName
    if tracker.get_token() in [";"]:
        tracker.advance() # write semicolon
        return token_list, current_idx, xml, global_symbols
    elif tracker.get_token() in [")"]:
        return token_list, current_idx, xml, global_symbols
    elif tracker.get_token() == ",":
        tracker.advance() # write comma
        return compile_varName_classVarDec(token_list, current_idx, xml, global_symbols=global_symbols)


def compile_classVarDec(token_list, current_idx, xml, global_symbols):
    if tracker.get_token() in ["constructor", "function", "method"]:
        ...
        token_list, current_idx, xml, global_symbols = compile_varName_classVarDec(token_list, current_idx, xml, global_symbols)
        ...
        return compile_classVarDec(token_list, current_idx, xml, global_symbols=global_symbols)

def compile_varDec(token_list, current_idx, xml, subroutine_symbols):
    if tracker.get_token() in ["let", "if", "while", "do", "return"]:
        ...
        token_list, current_idx, xml, subroutine_symbols = compile_varName(token_list, current_idx, xml, subroutine_symbols=subroutine_symbols, use_compile_type = False)
        ...
        return compile_varDec(token_list, current_idx, xml, subroutine_symbols=subroutine_symbols)

def compile_parameterList(token_list, current_idx, xml, subroutine_symbols):
    xml = write_partial_token(xml, name = "parameterList", position="beginning")
    token_list, current_idx, xml, subroutine_symbols = compile_varName(token_list, current_idx, xml, subroutine_symbols=subroutine_symbols, use_compile_type = True)
    xml = write_partial_token(xml, name = "parameterList", position="end")
    tracker.advance() # write )
    return token_list, current_idx, xml, subroutine_symbols

def compile_term(token_list, current_idx, xml):
    if tracker.get_token() == "(":
        tracker.advance() # write (
        token_list, current_idx, xml = compile_expression(token_list, current_idx, xml)
        tracker.advance() # write )
    elif tracker.get_token() in ["~", "-"]:
        tracker.advance() # write op
        xml = write_partial_token(xml, name = "term", position="beginning")
        token_list, current_idx, xml = compile_term(token_list, current_idx, xml)
        ...
        token_list, current_idx, xml = compile_subroutineCall(token_list, current_idx, xml)
    elif token_list[current_idx +1] == ".":
        token_list, current_idx, xml = compile_subroutineCall(token_list, current_idx, xml)
    else:
        tracker.advance() # write varname/integerconstant/stringconstant/keyboardconstant
    return token_list, current_idx, xml

def compile_expressionList(token_list, current_idx, xml):
    if tracker.get_token() == ")":
        return token_list, current_idx, xml
    else:
        if tracker.get_token() == ",":
            tracker.advance() # write comma
        token_list, current_idx, xml = compile_expression(token_list, current_idx, xml)
        return compile_expressionList(token_list, current_idx, xml)

def compile_expression(token_list, current_idx, xml):
    ...
    if tracker.get_token() in symbols[9:]:
        ...
        return compile_expression(token_list, current_idx, xml)       
    else:
        xml = write_partial_token(xml, name = "expression", position="end")
        return token_list, current_idx, xml

        
def compile_subroutineCall(token_list, current_idx, xml):
    ...
    if tracker.get_token() == "(":
        ...
        if tracker.get_token() == ")":
            ...
            xml = write_partial_token(xml, name = "expressionList", position="beginning")
            token_list, current_idx, xml = compile_expressionList(token_list, current_idx, xml)
            xml = write_partial_token(xml, name = "expressionList", position="end")
...
def compile_let(token_list, current_idx, xml):
...
def compile_ifwhile(token_list, current_idx, xml, name = ""):
    xml = write_partial_token(xml, name = f"{name}Statement", position="beginning")
    tracker.advance(2) # write if, (
    token_list, current_idx, xml = compile_expression(token_list, current_idx, xml)
    tracker.advance() # write ), {
    _, token_list, current_idx, xml = compile_Statements(token_list, current_idx, xml)
    tracker.advance() # write }
...
def compile_do(token_list, current_idx, xml):
...
def compile_return(token_list, current_idx, xml):
...
def compile_Statement(let_counter, token_list, current_idx, xml):
    if tracker.get_token() not in ["let", "if", "while", "do", "return", "else"]:
        return let_counter, token_list, current_idx, xml
    else:
        if tracker.get_token() == "let":
            let_counter += 1
            token_list, current_idx, xml = compile_let(token_list, current_idx, xml)
        if tracker.get_token() == "if":
            token_list, current_idx, xml = compile_ifwhile(token_list, current_idx, xml, name = "if")
        if tracker.get_token() == "while":
            token_list, current_idx, xml = compile_ifwhile(token_list, current_idx, xml, name = "while")
        if tracker.get_token() == "do":
            token_list, current_idx, xml = compile_do(token_list, current_idx, xml)
        if tracker.get_token() == "return":
            token_list, current_idx, xml = compile_return(token_list, current_idx, xml)
        return compile_Statement(let_counter, token_list, current_idx, xml)

def compile_Statements(token_list, current_idx, xml):
    let_counter = 0
    xml = write_partial_token(xml, name = "statements", position="beginning")
    let_counter, token_list, current_idx, xml = compile_Statement(let_counter, token_list, current_idx, xml)
    xml = write_partial_token(xml, name = "statements", position="end")
    return let_counter, token_list, current_idx, xml

def compile_subroutineBody(token_list, current_idx, xml, subroutine_symbols):
    ...
    token_list, current_idx, xml, subroutine_symbols = compile_varDec(token_list, current_idx, xml, subroutine_symbols)
    let_counter, token_list, current_idx, xml = compile_Statements(token_list, current_idx, xml)
    return let_counter, token_list, current_idx, xml, subroutine_symbols

def compile_subroutineDec(vm_code, token_list, current_idx, xml, subroutine_symbols):
    if tracker.get_token() not in ["constructor", "function", "method"]:
        return vm_code, token_list, current_idx, xml, subroutine_symbols
    else:
        ...
        if tracker.get_token() != ")":
            ...
            token_list, current_idx, xml, subroutine_symbols = compile_parameterList(token_list, current_idx, xml, subroutine_symbols=subroutine_symbols)
        ...
        tracker.advance() # write {  
        let_counter, token_list, current_idx, xml, subroutine_symbols = compile_subroutineBody(token_list, current_idx, xml, subroutine_symbols) 
        if is_constructor:
        ...
        subroutine_symbols = {"name": [], "type": [], "kind": [], "number": []}
        return compile_subroutineDec(vm_code, token_list, current_idx, xml, subroutine_symbols=subroutine_symbols)

def compile_class(token_list, xml, current_idx, global_symbols, subroutine_symbols):
    vm_code = []
    tracker.advance(3) # write class, className, {
    token_list, current_idx, xml, global_symbols = compile_classVarDec(token_list, current_idx, xml, global_symbols)
    vm_code, token_list, current_idx, xml, subroutine_symbols = compile_subroutineDec(vm_code, token_list, current_idx, xml, subroutine_symbols=subroutine_symbols)
    vm_code, token_list, current_idx, xml, subroutine_symbols = compile_subroutineDec(vm_code, token_list, current_idx, xml, subroutine_symbols=subroutine_symbols)
    tracker.advance() # write }
    print(vm_code)
    return vm_code, xml
